...
/**
 * Integration tests for {@link LsmVectorIndex} with all storage backed by S3. Uses
 * {@link PassthroughBloomFilter} (avoids alignment issues) and eager SSTable reads.
 */
...
class LsmVectorIndexS3Test {
    ...
    void ivfFlat_indexAndSearchReturnsExactMatch() throws IOException {
        try (VectorIndex.IvfFlat<Long> index = LsmVectorIndex.<Long>ivfFlatBuilder()
                .lsmTree(buildTree(Long.MAX_VALUE)).docIdSerializer(LONG_DOC_ID).dimensions(2)
    ...
    void ivfFlat_searchAfterFlush() throws IOException {
        try (VectorIndex.IvfFlat<Long> index = LsmVectorIndex.<Long>ivfFlatBuilder()
                .lsmTree(buildTree(1L)).docIdSerializer(LONG_DOC_ID).dimensions(2)
                .similarityFunction(SimilarityFunction.COSINE).build()) {
    ...
    void latencyBaseline_fiftyVectorsIndexAndSearch() throws IOException {
        int count = 50;
        try (VectorIndex.IvfFlat<Long> index = LsmVectorIndex.<Long>ivfFlatBuilder()
                .lsmTree(buildTree(Long.MAX_VALUE)).docIdSerializer(LONG_DOC_ID).dimensions(4)
                .similarityFunction(SimilarityFunction.COSINE).build()) {
