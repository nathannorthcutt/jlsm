...
class ContractBoundariesAdversarialTest {
    ...
    // Correct behavior: validateBase() should throw NullPointerException with message when precision is null
    ...
    // Finding: F-R5.cb.1.2
    // Bug: IvfFlat and Hnsw constructors guard all parameters with assert only — no runtime validation.
    ...
    // Fix location: LsmVectorIndex.java, IvfFlat constructor (lines 393-409), Hnsw constructor (lines 742-761)
    // Regression watch: Ensure runtime checks use Objects.requireNonNull and explicit IAE, not assert
    @Test
    void test_IvfFlat_constructor_rejects_null_precision_with_runtime_check() throws Exception {
        ...
        try {
            constructor.newInstance(lsmTree, LONG_DOC_ID_SERIALIZER, 4,
                    SimilarityFunction.COSINE, 8, 2, null);
            fail("Constructor should reject null precision");
        } catch (java.lang.reflect.InvocationTargetException e) {
            Throwable cause = e.getCause();
            assertInstanceOf(NullPointerException.class, cause,
                    "Expected NullPointerException for null precision, got " + cause.getClass().getName());
    ...
    void test_Hnsw_constructor_rejects_null_precision_with_runtime_check() throws Exception {
        ...
        try {
            constructor.newInstance(lsmTree, LONG_DOC_ID_SERIALIZER, 4,
                    SimilarityFunction.COSINE, 16, 200, 50, null);
            fail("Constructor should reject null precision");
        } catch (java.lang.reflect.InvocationTargetException e) {
            Throwable cause = e.getCause();
            assertInstanceOf(NullPointerException.class, cause,
    ...
    void test_IvfFlat_search_filters_infinity_scores() throws Exception {
        ...
        var index = LsmVectorIndex.<Long>ivfFlatBuilder()
                .lsmTree(lsmTree)
                .docIdSerializer(LONG_DOC_ID_SERIALIZER)
                .dimensions(4)
    ...
    // Fix location: LsmVectorIndex.java, Hnsw.searchLayer (lines 1003, 1024) and Hnsw.search (line 919)
    ...
    void test_Hnsw_search_filters_infinity_scores() throws Exception {
        ...
        // Use DOT_PRODUCT with FLOAT32 precision so that large component values
        // cause score overflow to Float.POSITIVE_INFINITY.
        var index = LsmVectorIndex.<Long>hnswBuilder()
                .lsmTree(lsmTree)
                .docIdSerializer(LONG_DOC_ID_SERIALIZER)
                .dimensions(4)
                .similarityFunction(SimilarityFunction.DOT_PRODUCT)
    ...
    void test_Hnsw_searchLayer_infinity_does_not_displace_valid_candidates() throws Exception {
        ...
        assertEquals(2, results.size(),
                "Expected 2 valid results but got " + results.size()
    ...
    // Correct behavior: ScoredCandidate should reject non-finite scores with IllegalArgumentException
    ...
    void test_ScoredCandidate_rejects_infinity_score() throws Exception {
        ...
        try {
            ...
            assertInstanceOf(IllegalArgumentException.class, cause,
                    "Expected IllegalArgumentException for Infinity score, got " + cause.getClass().getName());
        }
        ...
        try {
            ...
            Throwable cause = e.getCause();
            assertInstanceOf(IllegalArgumentException.class, cause,
                    "Expected IllegalArgumentException for NaN score, got " + cause.getClass().getName());
        }
