 ...
 *
 * @spec engine.clustering.R8 — message = (type, sender, monotonic sequence, opaque payload); payload is
 *       defensively copied on construction to prevent external mutation
 * @spec engine.clustering.R10 — exposes monotonic sequenceNumber so receivers that care about exactly-once
 *       semantics can deduplicate (deduplication is not enforced at protocol level)
