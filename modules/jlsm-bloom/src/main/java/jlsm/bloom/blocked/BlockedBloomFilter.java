 ...
 *
 * <p><b>Pipeline position:</b> constructed during SSTable flush (one filter per SSTable), then
 * serialized into the SSTable footer. On the read path, the filter is deserialized and consulted
 * before performing random I/O.
 *
 * <p><b>Algorithm:</b> uses MurmurHash3 x64-128 to derive two 64-bit seeds {@code (h1, h2)}, then
 * applies double hashing: {@code bitPos = (h1 + i * h2) & 511} for {@code i} in {@code [0, k)}.
 * All probes for a single key land within the same 512-bit block, selected by
 * {@code blockIndex = (h1 & Long.MAX_VALUE) % numBlocks}.
 *
 * <p><b>Why {@code h2} must be forced odd:</b> the bit positions form an arithmetic sequence
 ...
 *
 * <p><b>Serialization format (big-endian):</b>
 * <pre>
 ...
 *
 * <p><b>Thread-safety:</b> not thread-safe during the construction ({@link #add}) phase. Once
...
public final class BlockedBloomFilter implements BloomFilter {
    ...
    public BlockedBloomFilter(int expectedInsertions, double targetFpr) {
        ...
        if (targetFpr <= 0.0 || targetFpr >= 1.0) {
            throw new IllegalArgumentException("targetFpr must be in (0.0, 1.0), got: " + targetFpr);
        }
     ...
     * {@inheritDoc}
     *
     * <p>Computes the theoretical false-positive rate given the number of insertions so far:
     ...
     * {@inheritDoc}
     *
     * <p>Serialized layout (big-endian):
     * <pre>
     *   [int  numBlocks       ]  4 bytes
     *   [int  numHashFunctions]  4 bytes
     *   [long insertionCount  ]  8 bytes
    ...
    public static BloomFilter.Deserializer deserializer() {
        return bytes -> {
            ...
            for (int i = 0; i < bits.length; i++) {
                bits[i] = bytes.get(ValueLayout.JAVA_LONG.withOrder(ByteOrder.BIG_ENDIAN), 16L + i * 8L);
            }
