 ...
 */
// @spec partitioning.table-partitioning.R1 — public record in jlsm.table with the five documented components
public record PartitionDescriptor(long id, MemorySegment lowKey, MemorySegment highKey,
        String nodeId, long epoch) {

    // @spec partitioning.table-partitioning.R2,R3,R4,R5,R6,R7,R8 — null-reject (R2-R4), epoch >= 0 (R5), defensive copy (R6),
     ...
     */
    // @spec partitioning.table-partitioning.R9,R10 — content-based equals (R9); hashCode consistent below (R10)
    @Override
