...
/**
 * Service Provider Interface (SPI) the encryption layer consumes to wrap and unwrap tier-2 domain
 * KEKs under tier-1 tenant KEKs. The default reference implementation is {@code LocalKmsClient} in
 * {@code jlsm.encryption.local}; production deployments typically plug in an AWS KMS, GCP KMS, or
 * Vault Transit adapter.
 *
 * <p>
 * Implementations MUST:
 * <ul>
 * <li>Classify failures into the {@link KmsException} hierarchy so callers can distinguish
 * retryable from non-retryable errors (R76a).</li>
 * <li>Treat the provided {@link EncryptionContext} as Additional Authenticated Data binding the
 * wrap to its purpose and scope (R80, R80a).</li>
 * <li>Return a non-null {@link WrapResult#kekRef} recording the concrete KEK version used (R80b),
 * even if the input {@link KekRef} was an alias.</li>
 * </ul>
 *
 * <p>
 * Governed by: spec {@code encryption.primitives-lifecycle} R80, R80a, R80b; ADR
...
public interface KmsClient extends AutoCloseable {
    ...
    /**
     * Liveness/availability probe for a KMS KEK. Implementations SHOULD perform a lightweight call
     * (e.g., {@code DescribeKey} on AWS KMS) rather than a real wrap/unwrap.
     *
    ...
    /**
     * Release underlying resources. Implementations SHOULD be idempotent. Never propagates a
     * checked exception (matches {@link AutoCloseable} contract as narrowed).
     */
    @Override
    void close();
}
