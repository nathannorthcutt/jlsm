 ...
 *      R4a, R5</a>
 */
// @spec compression.zstd-dictionary.R4 — class-load detection via Panama FFM, catches all
// Throwable, cached
// @spec compression.zstd-dictionary.R4a — activeTier() exposes Tier enum for operator visibility
// @spec compression.zstd-dictionary.R5 — binds all required ZSTD downcall handles; fall through to
// Tier 2 on failure
final class ZstdNativeBindings {
    ...
    static {
        ...
        try {
            ...
            // @spec compression.zstd-codec.R1 — native detection failed; fall through to Tier 2
            // (pure-Java
