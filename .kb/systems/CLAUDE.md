# Systems — Topic Index

> **Managed by vallorcine agents. Use slash commands to modify this file.**
> To add research: `/research systems <category> "<subject>"`

Infrastructure, databases, distributed systems, caching, and data management strategies

## Categories

| Category | Path | Files | Last Updated | Description |
|----------|------|-------|--------------|-------------|
| vector-partitioning | [vector-partitioning/](vector-partitioning/CLAUDE.md) | 5 | 2026-03-16 | Strategies for partitioning large vector datasets across bounded-memory partitions |
| lsm-index-patterns | [lsm-index-patterns/](lsm-index-patterns/CLAUDE.md) | 24 | 2026-04-11 | How index types (inverted, vector, secondary) use LSM scan paths and cache |
| security | [security/](security/CLAUDE.md) | 8 | 2026-04-23 | Key management, three-level key hierarchy, key rotation, WAL encryption, SSTable block-level ciphertext envelope, DEK revocation semantics, DEK caching policies, client-side encryption SDK |
| database-engines | [database-engines/](database-engines/CLAUDE.md) | 12 | 2026-04-24 | Persistence, WAL, corruption, handles, schema types, cross-table transactions, pool-aware block sizing, format-version deprecation strategies |
| query-processing | [query-processing/](query-processing/CLAUDE.md) | 3 | 2026-03-30 | Join algorithms, anti-patterns, snapshot consistency for LSM-backed query execution |

## Navigation
Read the category CLAUDE.md to see individual subjects and comparisons.
Use /kb lookup systems <category> <subject> to load a specific entry.

## Research Gaps
<!-- Added by the Research Agent as categories are populated -->
